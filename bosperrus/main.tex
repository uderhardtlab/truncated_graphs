...
\subsection*{\texttt{bosperrus} concept}

\texttt{bosperrus} implements a modular pipeline for detecting, quantifying, and correcting border effects in spatial graphs.
The central object is the \texttt{Flow} class, which accepts per-node border distances and centrality scores and exposes corrected centralities, per-measure model summaries, and the fitted model objects.
To accommodate different levels of pre-processing, \texttt{Flow} provides four class-method constructors:
\texttt{from\_coords} builds the proximity graph and computes border distances from raw node coordinates;
\texttt{from\_coords\_and\_edgelist} accepts a pre-specified edge list and skips graph construction;
\texttt{from\_coords\_and\_scores} operates on pre-computed centrality scores; and
\texttt{from\_distances\_and\_scores} takes purely tabular input without requiring node coordinates, enabling integration with existing pipelines.

When node coordinates are available, border distances $d_n$ are computed using one of four built-in functions:
Euclidean distance to the convex hull of all node positions (\texttt{distance\_to\_convex\_hull});
minimum Euclidean distance to any side of the axis-aligned bounding rectangle (\texttt{distance\_to\_rectangular\_border});
nearest-neighbor distance to a user-supplied point set representing the border geometry (\texttt{distance\_to\_pointset}); or
distance-transform distance to a binary mask (\texttt{distance\_to\_mask}).
The choice is determined by the border geometry of the application at hand.

For each centrality measure $C^{(m)}$, \texttt{bosperrus} fits the four parametric models described in the following subsection to the set of observed pairs $\{(d_n,\,C_n^{(m)})\}$, after excluding nodes with non-finite distances or scores.
The three saturation models are fitted by nonlinear least squares using \texttt{scipy.optimize.curve\_fit}; for the piecewise linear model a subsequent \texttt{scipy.optimize.differential\_evolution} step refines the solution to reduce sensitivity to local minima.
The constant fit serves as the null model representing the absence of any border effect.
The best model is selected by the Akaike Information Criterion (AIC), computed as $\mathrm{AIC} = 2(k+1) - 2\ell$, where $k$ denotes the number of shape parameters, the additional $+1$ accounts for the residual variance $\widehat{\sigma}^2$ treated as an estimated parameter, and $\ell$ is the maximized log-likelihood (see Quality of fits below).
If the constant fit achieves the lowest AIC, no correction is applied.

When a saturation model is selected, \texttt{bosperrus} applies an additive correction that shifts each node's centrality to the plateau of the fitted model:
\begin{equation}
...
Nodes for which fitting failed fall back to the uncorrected value $\hat{C}_n = C_n$.
Corrected centralities are stored in \texttt{flow.observations} alongside the original scores; per-measure fit statistics---including $R_{\mathrm{scaled}}$, $H_{\mathrm{AIC}}$, effect strength, and half-life---are accessible via \texttt{flow.fit\_quality}; and the fitted model objects are stored in \texttt{flow.best\_fits}.
\texttt{bosperrus} is implemented in Python and is available on PyPI (\texttt{pip install bosperrus}).
